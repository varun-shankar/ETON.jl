...
function GraphStruct(i::AbstractVector,j::AbstractVector,p::AbstractArray)
    N = size(p,1)
    NN = dropdims(sum(sparse(i,j,1),dims=2),dims=2).+1
    rv = p[i,:].-p[j,:]
...
function (e::Eton00)(X::AbstractArray{T}) where {T}
    ...
    # Non-linearity
    out = e.σ.((Y1.+Y2)./e.gs.NN .+ tr(e.bias))
    return out
end
...
"""
struct Eton01{T,Po,Fi,R,F,G<:GraphStruct}
    weight::AbstractArray{T}
    bias::AbstractVector{T}
    pointwise::Po 
    filter::Fi # 2 x fo
...
function Eton01(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, pointwise, radial, σ = identity;
    init=Flux.glorot_uniform, T::DataType=Float32, bias::Bool=false)
    b = bias ? T.(init(ch[2])) : zeros(T, ch[2])
    Eton01(T.(init(ch[1], ch[2])), b, pointwise, T.(init(2, ch[2])), radial, σ, gs, ch)
end
...
function Eton01(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, hl::Int = 16, 
    ...
    pointwise = Chain(Dense(ch[1],hl,γ), Dense(hl,ch[2]))
    radial = Chain(Dense(1,hl,γ), Dense(hl,ch[2]))
    Eton01(gs, ch, pointwise, radial, σ; kwargs...)
end
...
function (e::Eton01)(X::AbstractArray{T}) where {T}
    ...
    Y2 = Xo[e.gs.j,:].*G # NNZ x fo x 2
    # Convolve
    Y2 = Flux.stack([e.gs.sp].*[Y2[:,:,i] for i=1:2],3) # N x fo x 2
    Y = (Y1.+Y2)./e.gs.NN
    # Non-linearity
    out = e.σ.(sqrt.(sum(Y.^2,dims=3)) .+ tr(e.bias)).*Y
    return out
end
...
Layer operations:
    ...
    6. Apply NL
"""
struct Eton10{T,Po,Fi,R,F,G<:GraphStruct}
    weight::AbstractArray{T}
    bias::AbstractVector{T}
    pointwise::Po 
    filter::Fi
...
function Eton10(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, pointwise, radial, σ = identity;
    init=Flux.glorot_uniform, T::DataType=Float32, bias::Bool=false)
    b = bias ? T.(init(ch[2])) : zeros(T, ch[2])
    Eton10(T.(init(ch[1], ch[2])), b, pointwise, T.(init(2, ch[2])), radial, σ, gs, ch)
end
...
function Eton10(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, hl::Int = 16, 
    ...
    pointwise = Chain(Dense(ch[1],hl,γ), Dense(hl,ch[2]))
    radial = Chain(Dense(1,hl,γ), Dense(hl,ch[2]))
    Eton10(gs, ch, pointwise, radial, σ; kwargs...)
end
...
function (e::Eton10)(X::AbstractArray{T}) where {T}
    ...
    # Apply pointwise weighting
    Y1 = sqrt.(dropdims(sum(Xo.^2,dims=3),dims=3)).*tr(e.pointwise(sqrt.(dropdims(sum(X.^2,dims=3),dims=3))|>tr))
    # Rotate each filter
    filter = e.filter./(sqrt.(sum(e.filter.^2,dims=1)))
    F = Flux.stack(batchmul.([rot(e.gs.θ)],Flux.unstack(adim(filter,2),3)),4) # 2 x 1 x NNZ x fo
    F = dropdims(permutedims(F,[3,4,1,2]),dims=4) # NNZ x fo x 2
    ...
    Y2 = dropdims(sum(Xo[e.gs.j,:,:].*G,dims=3),dims=3) # NNZ x fo
    # Convolve
    Y2 = e.gs.sp*Y2 # N x fo
    Y = (Y1.+Y2)./e.gs.NN
    # Non-linearity
    out = e.σ.(Y .+ tr(e.bias))
    return out
...
Layer operations:
    ...
    6. Apply NL to magnitude of the output features
"""
struct Eton11{T,Po,Fi,R,F,G<:GraphStruct}
    weight::AbstractArray{T}
    bias::AbstractVector{T}
    pointwise::Po 
    filter::Fi
...
function Eton11(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, pointwise, radial, σ = identity;
    init=Flux.glorot_uniform, T::DataType=Float32, bias::Bool=false)
    b = bias ? T.(init(ch[2])) : zeros(T, ch[2])
    Eton11(T.(init(ch[1], ch[2])), b, pointwise, T.(init(2,2, ch[2])), radial, σ, gs, ch)
end
...
function Eton11(gs::GraphStruct, ch::Pair{<:Integer,<:Integer}, hl::Int = 16, 
    ...
    pointwise = Chain(Dense(ch[1],hl,γ), Dense(hl,ch[2]))
    radial = Chain(Dense(1,hl,γ), Dense(hl,ch[2]))
    Eton11(gs, ch, pointwise, radial, σ; kwargs...)
end
...
function (e::Eton11)(X::AbstractArray{T}) where {T}
    ...
    Xo = batchmul(X,e.weight) # N x fo x 2
    # Apply pointwise weighting
    Y1 = Xo.*tr(e.pointwise(sqrt.(dropdims(sum(X.^2,dims=3),dims=3))|>tr))
    # Rotate each filter
    ...
    Y2 = batchmul(permutedims(G,[3,4,1,2]),permutedims(adim(Xo[e.gs.j,:,:]),[3,4,1,2])) # 2 x 1 x NNZ x fo
    Y2 = dropdims(permutedims(Y2,[3,4,1,2]),dims=4) # NNZ x fo x 2
    # Convolve
    Y2 = Flux.stack([e.gs.sp].*[Y2[:,:,i] for i=1:2],3)./e.gs.NN # N x fo x 2
    Y = (Y1.+Y2)./e.gs.NN
    # Non-linearity
    out = e.σ.(sqrt.(sum(Y.^2,dims=3)) .+ tr(e.bias)).*Y
    return out
